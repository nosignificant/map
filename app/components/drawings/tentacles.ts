...
//target이 있을 때 할 행동
export function FABRIKfollow(p: p5, t: Tentacle): [number, number][] {
  ...
  for (let i = newParts.length - 2; i >= 1; i--) {
    ...
    newParts[i] = [finalPos.x, finalPos.y];
  }
...
export function FABRIKswimm(p: p5, t: Tentacle): [number, number][] {
  ...
  for (let i = 1; i < body.length; i++) {
    ...
    const off = Math.sin(phase) * amp * (i / body.length); // 끝쪽일수록 더 크게
    body[i][0] += perp[0] * off;
    body[i][1] += perp[1] * off;
  }
...
export function drawFABRIK(p: p5, t: Tentacle, time: number) {
  t.t += time;
  const newParts = t.target != null ? FABRIKfollow(p, t) : FABRIKswimm(p, t);
  if (newParts.length > 0) t.parts = newParts;
  console.log("newParts", newParts.length, "parts", t.parts.length);
